...
/**
 *  PowerManager – Gestore intelligente dei carichi elettrici
 *  ---------------------------------------------------------
 *  Questa classe implementa un sistema avanzato di gestione dei carichi
 *  domestici/industriali basato su:
 *    • previsione della produzione solare
 *    • limiti di potenza prelevabile dalla rete
 *    • priorità dei carichi
 *    • isteresi dinamica
 *    • auto‑tuning dei parametri
 *    • suggerimenti automatici (attacca/stacca)
 *    • gestione carichi termici (riscaldamento/raffrescamento)
 *
 *  ---------------------------------------------------------
 *  1. INIZIALIZZAZIONE
 *  ---------------------------------------------------------
 ...
 *
 *  Parametri:
 *      gridLimitWatt     → limite massimo di potenza prelevabile dalla rete
 *      nominalSolarWatt  → potenza nominale dell’impianto FV
 *      hysteresisOn      → margine per attivare carichi
 *      hysteresisOff     → margine per disattivare carichi
 *
 *
 *  ---------------------------------------------------------
 *  2. REGISTRAZIONE CALLBACK
 *  ---------------------------------------------------------
 *
 ...
 *      onLoadChange     → notifica ogni cambio stato carico
 *      onLimitWarning   → avviso quando si sta per superare il limite
 *      onLimitExceeded  → limite superato
 *      onError          → errori sensori o parametri
 *      onSuggestion     → suggerimenti “attacca:” / “stacca:”
 *
 *  Nota:
 *      Se autoExecuteSuggestions è attivo, i suggerimenti vengono eseguiti
 ...
 *      minOnSec      → tempo minimo acceso
 *      minOffSec     → tempo minimo spento
 *
 *
 *  Carichi termici:
 *
 ...
 *
 *  Parametri:
 *      heatingMode   → true = riscaldamento, false = raffrescamento
 *      baseTarget    → temperatura target base
 *      comfortMin    → limite minimo comfort
 *      comfortMax    → limite massimo comfort
 *
 *
 *  ---------------------------------------------------------
 *  4. AGGIORNAMENTO DATI DI INPUT
 *  ---------------------------------------------------------
 *
 *      pm.setGridPower(wattPrelievo);
 *      pm.setSolarPower(wattFV);
 *      pm.setEnvironmentalData(lux, temperaturaEsterna);
 *
 *  Note:
 *      • lux viene usato per stimare la volatilità della produzione
 *      • temperatura esterna corregge la resa FV
 *
 *
 *  ---------------------------------------------------------
 *  5. CICLO PRINCIPALE (DA CHIAMARE PERIODICAMENTE)
 *  ---------------------------------------------------------
 *
 ...
 *
 *  Funzioni:
 *      updateLoads()         → gestisce attacco/stacco carichi normali
 *      updateThermalControl()→ gestisce carichi termici
 *      autoTuneStep()        → aggiorna isteresi e minOn/minOff
 *
 *
 *  ---------------------------------------------------------
 *  6. MODALITÀ DI OTTIMIZZAZIONE
 *  ---------------------------------------------------------
 *
 *      pm.setOptimizationMode(PowerManager::OptimizationMode::MASSIMO_AUTOCONSUMO);
 *
 *  Modalità disponibili:
 *      MASSIMO_AUTOCONSUMO  → aumenta il limite dinamico
 *      RISPARMIO_ECONOMICO  → riduce il limite dinamico
 *      MASSIMO_COMFORT      → aggiunge margine fisso
 *      PROTEZIONE_RETE      → limita fortemente il prelievo
 ...
 *  ---------------------------------------------------------
 *
 *  Il sistema genera stringhe del tipo:
 *
 *      "attacca:Lavatrice"
 *      "stacca:Forno"
 *
 *  Se autoExecuteSuggestions = true:
 *      → il carico viene attivato/disattivato automaticamente
 *
 *  Se autoExecuteSuggestions = false:
 *      → il suggerimento viene solo notificato via callback
 *
 *
 *  ---------------------------------------------------------
 *  8. AUTO‑TUNING
 ...
 *      pm.setTuneInterval(300000);     // 5 minuti
 *      pm.setTuneStepPercent(0.05f);   // 5%
 *
 *  L’auto‑tuning regola automaticamente:
 *      • isteresi on/off
 *      • minOn/minOff dei carichi
 *
 *  In base a:
 *      • numero cicli dei carichi
 *      • volatilità della luce (lux)
 *      • errore di forecast FV
 *
 *
 *  ---------------------------------------------------------
 *  9. FORECAST SOLARE
 ...
 *
 *  Basato su:
 *      • curva sinusoidale teorica
 *      • lux attuale
 *      • temperatura esterna
 *      • volatilità (correzione breve termine)
 *
 *
 *  ---------------------------------------------------------
 *  10. ERRORI
 *  ---------------------------------------------------------
 *
 *      pm.reportError(code, "messaggio");
 *
 *  Errori generati automaticamente:
 *      • valori sensori non validi
 ...
 *  ---------------------------------------------------------
 *
 *  La classe è pensata per essere chiamata ciclicamente (es. ogni 1–5 secondi).
 *  Tutta la logica di attacco/stacco è protetta da:
 *      • minOn/minOff
 *      • isteresi
 *      • rollback automatico se si supera il limite
 *
 *  Il sistema è modulare e può essere esteso con:
 *      • nuovi tipi di carichi
 *      • logiche di forecast più avanzate
 *      • integrazione con inverter o smart‑meter
 *
 */
