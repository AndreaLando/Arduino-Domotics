...
{
  public:            
    OneShot(bool initialState);   
    bool Loop(bool state);   //Da usarsi in alternativa a Set\Reset\Done\GetState\IsDone
    void Set();                 
    void Reset(); 
    void Done();   
    bool GetState();    
...
{
  public:   
    AntiBump(bool initialState);          
    bool Change(bool state);       
  private:     
    OneShot OS(bool initialState); 
    bool _initialState;
};
...
class Valve
{
  ...
  Valve(int address, unsigned long timeout);     
  void Run(int switchStatus);
  void Open();
  void Close(); 
  void Stop(); 
  bool IsInError(); 
